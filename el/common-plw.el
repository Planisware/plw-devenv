...
(defvar *copyright-string-closeness-threshold* 0.9)

(load (fullpath-relative-to-current-file "script-mode-copyright.el"))

(defun string-closeness (str1 str2)
  (let ((s 0.0)
	(n 0.0)
	(current-score 0.0)
	(current-total 0.0)
	...
	((or (> i l1)
	     (> j l2)))
      (let ((c1 (if (< i l1) (aref str1 i) ?\n))
	    (c2 (if (< j l2) (aref str2 j) ?\n)))
	(cond ((or (eq c1 ?\n)
		   (eq c2 ?\n))
	       (let ((line-score (cond ((eq c1 c2)		      
					(/ current-score current-total))
				       ((= l1 i)
					(/ current-score (+ current-total (- l2 j))))
				       ((= l2 j)
					(/ current-score (+ current-total (- l1 i))))
				       ((eq c1 ?\n)
					(prog1
					    (/ current-score (+ current-total (- (or (position ?\n str2 :start j) l2) j)))
					  (setq j (or (position ?\n str2 :start j) l2))))
				       ((eq c2 ?\n)
					(prog1
					    (/ current-score (+ current-total (- (or (position ?\n str1 :start i) l1) i)))
		 ...
		 (setq s (+ s line-score))
		 (setq n (1+ n))
		 (setq current-score 0.0)
		 (setq current-total 0.0)))
	      ((eq c1 c2)
	       (setq current-score (1+ current-score))
	       (setq current-total (1+ current-total)))
	      (t
	       (setq current-total (1+ current-total))))))
    (/ s n)))
...
(defun check-header (&optional force)
  ...
  (let* ((head *script-copyright-head*)
	 (strc (string-closeness (buffer-substring-no-properties 1 (min (point-max) (1+ (length head)))) head)))
    (when (or force
	      (and (< strc *copyright-string-closeness-threshold*)
